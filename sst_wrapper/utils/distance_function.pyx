...
class TriStateDistance(_sst_module.IDistance):
    ...
    def distance(self,double[:] point1,double[:] point2):
        cdef double distance = ((point1[0]-point2[0])**2 +(point1[1]-point2[1])**2)**0.5
        return distance + abs(normalize_angle(point1[2]-point2[2]))+ abs(normalize_angle(point1[5]-point2[5]))
